...
def main(test_mode=False):
    ...
    """
    with open(r"Training Data/plant_diagnostics_final.json", "r", encoding="utf-8") as f:
    ...
    customized_collate_fn = partial(custom_collate_fn, device=this_device, allowed_max_length=512)
    ...
    warmup = LinearLR(optimizer, start_factor=0.1, total_iters=int(0.1*total_steps))
    cosine = CosineAnnealingLR(optimizer, T_max=total_steps-int(0.1*total_steps))
            ...
            if global_step > 0 and global_step % eval_freq == 0 and accumulation_counter % gradient_accumulation_steps == 0:
                ...
                with torch.no_grad():
                    ...
                    token_ids = generate(
                                        ...
                                        max_new_tokens=200,context_size=BASE_CONFIG["context_length"], temperature=0.0,
                                        top_k=1, eos_id=50256, stop_strings = None, tokenizer=tokenizer)
    ...
    for i, entry in tqdm(enumerate(test_data), total=len(test_data)):
        ...
        token_ids = generate(
                            ...
                            max_new_tokens=200,context_size=BASE_CONFIG["context_length"], temperature=0.7,
                            top_k=40, eos_id=50256, stop_strings = None, tokenizer=tokenizer)
    ...
    test_data_path = "Training Data/Plant_diagnostics-with-response-standalone.json"
    ...
    file_name = f"gpt2/gpt-files/{re.sub(r'[ ()]', '', CHOOSE_MODEL)}-sft-standalone(p0).pth"
